...
export const removeEmployeeFromStore = onCall({ region: 'us-central1' }, async (request) => {
  ...
  try {
    ...
    if (employeeStoreSnap.exists) batch.delete(employeeStoreRef);
...
function stepFailed(uid: string, mode: string, step: string, error: unknown): HttpsError {
  const message = error instanceof Error ? error.message : String(error);
  console.error(`[AccountDelete] uid=${uid} mode=${mode} step=${step} failed message=${message}`);
  return new HttpsError('internal', 'step_failed', { ok: false, code: 'step_failed', step, message, uid, mode });
}

async function commitDeleteBatch(refs: FirebaseFirestore.DocumentReference[], context: { prefix: string; uid: string; stage: string }): Promise<number> {
...
export const deleteMyAccount = onCall({ region: 'us-central1' }, async (request) => {
  ...
  try {
    ...
    const userData = userSnap.data() ?? {};
    const assignedStoreIds = Array.isArray(userData.assignedStoreIds) ? userData.assignedStoreIds.filter((v: unknown): v is string => typeof v === 'string') : [];
    ...
    } catch (error) { throw stepFailed(uid, mode, 'employeeStores', error); }
    ...
    } catch (error) { throw stepFailed(uid, mode, 'employeeCheckins', error); }
    ...
    try {
      ...
      for (const doc of [...membershipsById.docs, ...membershipsByDoc.docs]) addRef(doc.ref);
      for (const storeId of assignedStoreIds) addRef(db.collection('stores').doc(storeId).collection('members').doc(uid));
      console.log(`[deleteMyAccount] uid=${uid} stage=memberships count=${membershipsById.size + membershipsByDoc.size}`);
    } catch (error) { throw stepFailed(uid, mode, 'memberships', error); }
    ...
    } catch (error) { throw stepFailed(uid, mode, 'managerCheckins', error); }
    ...
    } catch (error) { throw stepFailed(uid, mode, 'cleanupFirestore', error); }
...
export const deleteManagerAccount = onCall({ region: 'us-central1' }, async (request) => {
  ...
  if (!managerUid) throw new HttpsError('unauthenticated', 'Authentication required');
  ...
  try {
    ...
    if (role && role !== 'manager') {
      throw new HttpsError('permission-denied', 'Only managers can call deleteManagerAccount.', { ok: false, code: 'step_failed', step: 'resolveRole', message: 'non_manager' });
    }
...
async function sendManagerPushForCheckin(data: FirebaseFirestore.DocumentData, checkinId: string, kind: 'checkin' | 'checkout') {
...
export const notifyManagerOnCheckinCreate = onDocumentCreated({ region: 'us-central1', document: 'checkins/{checkinId}' }, async (event) => {
  const data = event.data?.data();
  if (!data) return;
  await sendManagerPushForCheckin(data, event.params.checkinId, 'checkin');
});

export const notifyManagerOnCheckoutUpdate = onDocumentUpdated({ region: 'us-central1', document: 'checkins/{checkinId}' }, async (event) => {
  const before = event.data?.before.data();
  const after = event.data?.after.data();
  if (!after) return;
  const checkoutAdded = !before?.checkOutTime && !!after.checkOutTime;
  if (!checkoutAdded) return;
